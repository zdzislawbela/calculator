...
export const handleKeyboardButtons = (userInput) => {
  ...
  if (userChooseDigit) return handleDigit(userInput);
  if (userChooseModifier) return handleModifier(userInput);
  if (userChooseSign) return handleDeletetionSign(userInput);
  if (userChooseOperator) return handleOperator(userInput);
};
...
const handleDigit = (digit) => {
  ...
  const addDigitBeforeParentheses = `${CALCULATOR_STATE.currentValue.slice(
    0,
    CALCULATOR_STATE.currentValue.length - 1
  )}${digit})`;
...
const handleModifier = (modifier) => {
  ...
  const removeMinusAtBegin = CALCULATOR_STATE.currentValue.slice(
    2,
    currentLength - 1
  );
  const addMinusAtBegin = `-(${CALCULATOR_STATE.currentValue})`;
  const removeParenthesesAndAddDot = `${CALCULATOR_STATE.currentValue.slice(
    0,
    currentLength - 1
  )}.)`;
...
const handleOperator = (operator) => {
  ...
  const isZeroAtTheEnd =
    parseInt(
      CALCULATOR_STATE.currentValue.slice(
        CALCULATOR_STATE.currentValue.indexOf(".") + 1,
        CALCULATOR_STATE.currentValue.length
      )
    ) === 0;
  const isDotAtTheEnd =
    CALCULATOR_STATE.currentValue.slice(
      CALCULATOR_STATE.currentValue.indexOf("."),
      CALCULATOR_STATE.currentValue.length
    ) === ".";
  const removeZeroOrDotAtTheEnd = CALCULATOR_STATE.currentValue.slice(
    0,
    CALCULATOR_STATE.currentValue.indexOf(".")
  );
...
const handleDeletetionSign = (deletetionSign) => {
  ...
  const assignZero = "0";
  const deleteLastDigit = CALCULATOR_STATE.currentValue.slice(
    0,
    CALCULATOR_STATE.currentValue.length - 1
  );

  pressedDel ? (CALCULATOR_STATE.currentValue = deleteLastDigit) : "";

  isCurrentValueEmpty ? (CALCULATOR_STATE.currentValue = assignZero) : "";

  //purge all displayRows if exists (not implemented)
  pressedC ? (CALCULATOR_STATE.currentValue = "0") : "";

  //purge last dislayRows (not implemented)
  pressedCE ? (CALCULATOR_STATE.currentValue = "0") : "";
